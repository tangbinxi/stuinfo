...
/*	fprintf(cgiOut, "<head><meta charset=\"utf-8\"/><title>查询结果</title>\
	...
	if (name[0] == '*')
	{
		sprintf(sql, "select * from stu,Score,Course where stu.id=Score.id and Course.Cid=Score.Cid and stu.sta=0");
	}
	else
	{
		sprintf(sql, "select * from stu,Course,Score where name = '%s' and stu.id=Score.id and Course.Cid=Score.Cid and stu.sta=0", name);
	}
